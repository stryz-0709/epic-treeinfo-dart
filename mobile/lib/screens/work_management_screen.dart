...
class _WorkManagementScreenState extends State<WorkManagementScreen> {
  List<MobileRangerStats>? _rangerStats;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (authProvider.isLeaderSession) {
        _loadRangerStats();
      }
    });
  }

  Future<void> _loadRangerStats() async {
    final authProvider = context.read<AuthProvider>();
    final api = context.read<MobileApiService>();
    final token = authProvider.mobileAccessToken?.trim() ?? '';
    if (token.isEmpty) return;

    setState(() => _loadingStats = true);
    try {
      final now = DateTime.now();
      final fromDay = DateTime(now.year, now.month, 1);
      final toDay = DateTime(now.year, now.month + 1, 0);
      final stats = await api.fetchWorkStats(
        accessToken: token,
        fromDay: fromDay,
        toDay: toDay,
      );
  ...
  Widget build(BuildContext context) {
    ...
    final leaderFilterValue =
        workProvider.selectedRangerId != null &&
            workProvider.availableRangerIds.contains(
    ...
    return Scaffold(
      ...
      appBar: AppTopToolbar(
        title: l.get('landing_function_work'),
                                ...
                                children: [
                                  ...
                                  Wrap(
                                    ...
                                    children: [
                                      _buildSyncCountChip(
                                        label: l.get(
                                          'work_sync_status_count_pending',
                                        ),
                                  ...
                                  if (workProvider.failedCheckinCount > 1) ...[
                                    ...
                                    Align(
                                      ...
                                      child: FilledButton.tonalIcon(
                                        ...
                                        onPressed:
                                            workProvider.isReplayingCheckins ||
                                                workProvider.isSyncingCheckin
                                            ? null
                                            : () async {
                                                await workProvider
                            ...
                            DropdownButtonFormField<String?>(
                              ...
                              items: [
                                DropdownMenuItem<String?>(
                                  value: null,
                                  child: Text(
                                    l.get('work_calendar_filter_all'),
                                  ),
                                ),
                            ...
                            Expanded(
                              child: Column(
                                children: [
                                  ...
                                  Expanded(
                                    child: GridView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        final total = workProvider
                                            .totalCountForDay(day);
                                        final checked = workProvider
                                            .checkinCountForDay(day);
                                  ...
                                  if (isLeader &&
                                      _rangerStats != null &&
                                      _rangerStats!.isNotEmpty) ...[
                                    const SizedBox(height: 10),
                                    Text(
                                      l.get('work_stats_title'),
                                      style: const TextStyle(
                                        fontSize: 13,
                                        fontWeight: FontWeight.w700,
                                        color: Color(0xFF1B2838),
                                    ...
                                    SizedBox(
                                      ...
                                      child: ListView.separated(
                                        ...
                                        itemCount: _rangerStats!.length,
                                        separatorBuilder: (_, __) =>
                                            const SizedBox(width: 8),
                                        itemBuilder: (ctx, idx) {
                                          final stat = _rangerStats![idx];
                                          final rate = stat.totalDays > 0
                                              ? (stat.checkinDays *
                                                        100 /
                                                        stat.totalDays)
                                                    .round()
                                          ...
                                          return Container(
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                const SizedBox(height: 4),
                                                Text(
                                                  '${l.get('work_stats_checkin_rate')}: $rate%',
                                                  style: TextStyle(
                                                    fontSize: 11,
                                                    color: rate >= 80
                                                        ? const Color(
                                                ...
                                                Text(
                                                  '${l.get('work_stats_incidents')}: ${stat.incidentsFound}',
                                                  style: const TextStyle(
                                                    fontSize: 11,
                                                    color: Color(0xFF475467),
                                                  ),
                                  ...
                                  ] else if (isLeader && _loadingStats) ...[
                                    const SizedBox(height: 8),
                                    const Center(
                                      child: SizedBox(
                                        width: 16,
