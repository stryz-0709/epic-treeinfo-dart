...
class _LinkTreeScreenState extends State<LinkTreeScreen> {
  ...
  Future<void> _linkTree(TreeModel tree) async {
    ...
    try {
      ...
      // Navigate to the resource screen and pre-load this tree by NFC ID
      Navigator.of(
        context,
      ).pushReplacementNamed('/resource-management', arguments: widget.nfcId);
    } catch (e) {
      if (!mounted) return;
      AppNotification.showTop(
        context,
